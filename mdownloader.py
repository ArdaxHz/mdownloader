    ...
    if not vargs["update"]:
        ...
        if root_response.status_code == 200 and components_response.status_code == 200:
            ...
            remote_version_info_response = requests.get(
                "https://raw.githubusercontent.com/Rudoal/mdownloader/master/components/__version__.py"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-l",
        default="en",
        help="Specify the language to download. NEEDED to download non-English chapters on manga downloads.",
    )
    parser.add_argument(
        "--type",
        "-t",
        ...
        nargs="?",
        const="manga",
        help="Type of id to download, manga, chapter, group, user, list or follows.",
    )
    parser.add_argument(
        "--directory",
        "-d",
        default=None,
        help="Directory where to download chapters, takes priority over whats in the env file.",
    )
    parser.add_argument(
        "--folder", "-f", default=False, const=True, nargs="?", help="Download into a folder instead of an archive."
    )
    parser.add_argument(
        "--covers",
        "-c",
        ...
        const=True,
        nargs="?",
        help="Download the covers of the manga. Works only with manga downloads.",
    )
    parser.add_argument(
        "--json",
        "-j",
        ...
        const=False,
        nargs="?",
        help="Add the chapter data as a json in the chapter archive/folder.",
    )
    parser.add_argument(
        "--range",
        "-r",
    ...
    parser.add_argument(
        ...
        const=True,
        nargs="?",
        help="Download chapters in descending order instead of grouping by manga.",
    )
    ...
    parser.add_argument(
        "--rename", default=True, const=False, nargs="?", help="Skip renaming downloaded files if the title is wrong."
    )

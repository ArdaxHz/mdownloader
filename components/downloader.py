...
async def manga_download(args: 'ProcessArgs', manga_args_obj: 'MDArgs', chapters: hondana.ChapterFeed=None) -> None:
    """Download manga."""
    ...
    manga_args_obj = dataclasses.replace(manga_args_obj, **{"cache": manga_cache_obj})

    if manga_args_obj.data or refresh_cache or not bool(manga_cache_obj.cache.data):
        if manga_args_obj.data:
            manga_data = manga_args_obj.data
        else:
    ...
    title_json = TitleJson(args, manga_args_obj)
